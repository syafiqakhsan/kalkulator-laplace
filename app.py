...
# 1. Set konfigurasi halaman agar penuh (Wide Mode)
st.set_page_config(page_title="Kalkulator Laplace RLC", layout="wide")

# 2. Kode HTML + JS + CSS utuh yang sudah dicleanup bebas crash syntax
html_code = """
<!DOCTYPE html>
<html lang="id">
<head>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .result-card .label {
    ...
    color: #4a5a8a; margin-bottom: 6px;
  }

  .result-card .value {
    font-size: 0.95rem; color: #93c5fd; font-weight: bold;
    word-break: break-all;
  }

  .result-card .value.formula {
  ...
  .steps-panel h3 {
    ...
    color: #4a5a8a; margin-bottom: 1rem;
  }
  ...
  .empty-state {
    ...
    color: #4a5a8a; font-size: 0.85rem;
  }
...
function calculate() {
  ...
  if (currentCircuit === 'RL') {
    result = calcRL(R, L, V, tmax);
  } else if (currentCircuit === 'RC') {
    result = calcRC(R, C, V, tmax);
  } else {
    result = calcRLC(R, L, C, V, tmax);
  }
...
function calcRL(R, L, V, tmax) {
  ...
  const tau = 1 / alpha;
  const timeData = linspace(0, tmax, 300);
  const currentData = timeData.map(iFunc);

  return {
    type: 'RL',
    formula: 'i(t) = ' + fmtNum(A) + ' − ' + fmtNum(Math.abs(B)) + '·e^(−' + fmtNum(alpha) + 't) A',
    params: { R, L, V },
    derived: { alpha: fmtNum(alpha), tau: fmtNum(tau), Iss: fmtNum(A) },
    steps, timeData, currentData,
    tableRows: buildTable(timeData, currentData),
    svgDiagram: svgRL(R, L, V),
    colorClass: 'badge-RL',
    iFunc, tmax
  };
}
...
function calcRC(R, C, V, tmax) {
  ...
  return {
    type: 'RC',
    formula: 'i(t) = ' + fmtNum(A) + '·e^(−' + fmtNum(alpha) + 't) A',
    params: { R, C, V },
    derived: { alpha: fmtNum(alpha), tau: fmtNum(tau), Iss: '0 (kapasitor penuh)' },
    steps, timeData, currentData,
    tableRows: buildTable(timeData, currentData),
    svgDiagram: svgRC(R, C, V),
    colorClass: 'badge-RC',
    iFunc, tmax
  };
}
...
function calcRLC(R, L, C, V, tmax) {
  ...
  return {
    ...
    formula,
    params: { R, L, C, V },
    derived: { b: fmtNum(b), disc: fmtNum(disc), regime: disc>0?'Overdamped':disc===0?'Critically Damped':'Underdamped' },
    steps, timeData, currentData,
    tableRows: buildTable(timeData, currentData),
    svgDiagram: svgRLC(R, L, C, V),
    colorClass: 'badge-RLC',
    iFunc, tmax
  };
}
...
function buildTable(t, i) {
  const rows = [];
  const steps = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60];
  steps.forEach(tv => {
    const idx = t.findIndex(x => x >= tv);
    if (idx >= 0) rows.push({ t: tv, i: i[idx] });
  });
  return rows;
}

function renderResults(r) {
  const colors = { RL: '#34d399', RC: '#f472b6', RLC: '#fbbf24' };
  const col = colors[r.type];

  const tableHTML = r.tableRows.map(row =>
    '<tr><td>' + row.t + '</td><td>' + fmtNum(row.i, 6) + '</td></tr>'
  ).join('');

  const stepsHTML = r.steps.map((s, i) => '    <div class="step">      <div class="step-num">' + (i+1) + '</div>      <div class="step-content">        <div class="step-title">' + s.title + '</div>        <div class="step-formula">' + s.formula + '</div>      </div>    </div>').join('');

  const derivedHTML = Object.entries(r.derived).map(([k,v]) =>
    '<div class="result-card"><div class="label">' + k + '</div><div class="value">' + v + '</div></div>'
  ).join('');

  document.getElementById('content').innerHTML = '    <div style="display:flex;align-items:center;gap:10px;margin-bottom:1.5rem;">      <span style="font-size:0.7rem;letter-spacing:2px;text-transform:uppercase;color:#4a5a8a;">Hasil:</span>      <span style="font-size:1rem;font-weight:bold;color:' + col + ';">Rangkaian ' + r.type + ' Seri</span>    </div>    <div class="circuit-diagram">' + r.svgDiagram + '</div>    <div class="results-grid">      <div class="result-card highlight" style="grid-column:1/-1;">        <div class="label">Persamaan Arus i(t) — Hasil Transformasi Laplace</div>        <div class="value formula" style="color:' + col + ';font-size:1rem;">' + r.formula + '</div>      </div>      ' + derivedHTML + '    </div>    <div class="steps-panel">      <h3>Langkah-langkah Penyelesaian</h3>      ' + stepsHTML + '    </div>    <div class="chart-container">      <h3>Grafik Arus i(t) terhadap Waktu</h3>      <canvas id="chart-canvas" height="260"></canvas>    </div>    <div class="table-panel">      <h3>Tabel Perbandingan Nilai i(t)</h3>      <table>        <thead>          <tr><th>t (detik)</th><th>i(t) — Transformasi Laplace (A)</th></tr>        </thead>        <tbody>' + tableHTML + '</tbody>      </table>    </div>';
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          bodyColor: '#e0e8ff',
          callbacks: {
            label: ctx => 'i(t) = ' + ctx.raw.toExponential(4) + ' A'
          }
        }
      },
      scales: {
        x: {
          ticks: { color: '#6b7db3', font: { family: 'Courier New', size: 11 }, maxTicksLimit: 10 },
          grid: { color: '#1e2d52' },
          title: { display: true, text: 'Waktu (detik)', color: '#6b7db3', font: { family: 'Courier New' } }
        },
        y: {
          ticks: { color: '#6b7db3', font: { family: 'Courier New', size: 11 }, callback: v => v.toExponential(2) },
          grid: { color: '#1e2d52' },
          title: { display: true, text: 'Arus i(t) [Ampere]', color: '#6b7db3', font: { family: 'Courier New' } }
        }
      }
    }
  });
...
function svgRLC(R, L, C, V) {
  return '<svg width="480" height="140" viewBox="0 0 480 140" xmlns="
